...
BEGIN_EVENT_TABLE(fDialog, wxDialog)
    EVT_CLOSE(fDialog::OnClose)
    EVT_BUTTON(idBtnQuit, fDialog::OnQuit)
    EVT_BUTTON(idBtnAbout, fDialog::OnAbout)
END_EVENT_TABLE()

fDialog::fDialog(wxDialog *dlg, const wxString &title)
...
void fDialog::OnClose(wxCloseEvent &event)
{
    Destroy();
}

void fDialog::OnQuit(wxCommandEvent &event)
{
    Destroy();
}

void fDialog::OnAbout(wxCommandEvent &event)
{
